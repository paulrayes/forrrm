...
var cheatSheetMarkdown;
if (process.browser) {
	cheatSheetMarkdown = require('./editorCheatSheet.md');
} else {
	var fs = require('fs');
	cheatSheetMarkdown = fs.readFileSync(__dirname + '/editorCheatSheet.md', 'utf8');
}
//var cheatSheetMarkdown = 'hello';
...
module.exports = function(React) {
	...
	return React.createClass({
		...
		componentDidMount: function componentDidMount() {
			require.ensure(['remarkable'], require => {
		...
		render: function render() {
			...
			if (typeof this.state.md !== 'undefined') {
				html = this.state.md.render(this.props.value);
			}
			return (
